...
def main() -> None:
    ...
    x_mkt_train: np.ndarray = x_mkt[:int(len(x_mkt) * 0.5)]
    y_mkt_train: np.ndarray = y_mkt[:int(len(y_mkt) * 0.5)]

    x_mkt_test: np.ndarray = x_mkt[int(len(x_mkt) * 0.5):]
    y_mkt_test: np.ndarray = y_mkt[int(len(y_mkt) * 0.5):]
